...
void signal_handler (int sig_num)
{
  ...
  if(dot_file.is_open())
  {
    ...
    {
      chk->print_U_shape();
      // chk->print_U_sequnece(chk->which_U(),std::cout);
      chk->bi_draw_graph();
    }
  }
  dot_file.close();
...
void check_aiger (int argc, char** argv)
{
  ...
  //get aiger object
   ...
   chk = new Checker(model, res_file, trail_file, dot_file, dive_file,enable_dive, forward, propagate, evidence, 0, enable_bi_check);

   bool res = chk->bi_check();
   delete chk;
